...
const optionsStatutBesoin = [
 ...
 <div className="flex flex-col  md:px-12 md:py-8 md:gap-8   md:grid md:grid-cols-3 p-4">
  ...
  <div className="  w-full md:p-4 mt-12 mb-4 md:mb-0 md:mt-0  order-1 md:col-span-2  ">
    ...
    <div className="flex justify-between items-center ">
      <div className="flex items-center gap-2">
        <div>
          {animal.sexe === 'Femelle' && (
            <FemelleIcon className="text-white bg-pink-500 text-lg p-[1.5px] rounded-sm shadow-md" title="Femelle" />
          )}
          {animal.sexe === 'Mâle' && (
            <MaleIcon className="text-white bg-blue-500 text-lg p-[1.5px] rounded-sm shadow-md" title="Mâle" />
          )}
        </div>
        <h1 className="text-3xl font-bold flex items-center gap-2">{animal.name}</h1>
      </div>

      <div className="flex gap-6">
        <button onClick={() => setShowModal(true)} className="underline px-4 py-2 flex items-center gap-2 text-sm font-medium">
          Archiver
          <Archive />
        </button>
  ...
  <div className="  w-full flex flex-col gap-4 mb-8 pb-[5rem] md:mb-0 bg-white shadow-lg rounded-2xl md:h-screen  order-2 md:row-span-2">
    <div className="relative rounded-2xl overflow-hidden shadow">
      <div className="absolute top-2 left-2 flex flex-wrap gap-2 z-10">
        ...
        {animal.rechercheFa && <span className="bg-white text-black text-xs font-medium px-3 py-1 rounded-full shadow">En recherche FA</span>}
        {animal.adoption && <span className="bg-white text-black text-xs font-medium px-3 py-1 rounded-full shadow">À l’adoption</span>}
      </div>
      <img src={`https://cyrvizynjvgblsmyntmc.supabase.co/storage/v1/object/public/photos/${animal.img}`} alt={animal.name} className="w-full h-[350px] object-cover" />
    </div>
    ...
    <div className="flex flex-wrap gap-2 mt-2 ml-4">
      ...
      <div className="flex flex-wrap gap-2 mt-2">
        {optionsStatutBesoin.map(({ label, key }) =>
          animal[key.trim()] ? (
            <span key={key} className="border border-gray-400 text-gray-700 text-xs font-medium px-2 py-1 rounded-[5px]">
              {label}
            </span>
          ) : null
        )}
    ...
    <div className="text-sm mt-2 flex flex-col gap-6 ml-4">
  ...
  className=" bg-white rounded-xl shadows-xl  p-6 relative overflow-visible  order-3 md:col-span-2 -mt-12">
